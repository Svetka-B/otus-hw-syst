...
output "elasticsearch_1_ip" {
  value = split("/", local.vms["es-1"].ip)[0]
}
output "elasticsearch_2_ip" {
  value = split("/", local.vms["es-2"].ip)[0]
}
output "elasticsearch_3_ip" {
  value = split("/", local.vms["es-3"].ip)[0]
}
output "logstash_1_ip" {
  value = split("/", local.vms["ls-1"].ip)[0]
}
output "kibana_1_ip" {
  value = split("/", local.vms["kibana-1"].ip)[0]
}
